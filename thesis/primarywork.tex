...
Given a sorted array $A[1..n]$ of $\omega$-bit integers, predecessor search queries in $\mathcal{O}(\lg \omega)$ time is supported using $\mathcal{O}(n \lg \omega)$ bits of space which has oracle access to the entries in the array. $\mathcal{O}(n \lg \omega)$ bits of space is not enough to store $n$ $\omega$-bit integers, so this data structure defers the look-up operation to an oracle machine. For this purpose a Patricia trie \cite{morehaste} is used to store the some parts of the $n$ $\omega$-bit integer and find the entry in the array $A$ which is to be looked up by the oracle machine.
