...
\begin{enumerate}
  \item We find the rank space successor (equal or greater) of $x_1$, $x^*_1$, and the rank space predecessor (equal or smaller) of $x_2$, $x^*_2$. We use these to find the lowest common ancestor of $x^*_1$ and $x^*_2$, $LCA(x^*_1, x^*_2)$. This is the lowest node in the tree containing at least all the points $x_1$ and $x_2$. By knowing $x^*_1$ and $x^*_2$, finding the lowest common ancestor is a constant time operation. Given that points are in an array, we can use xor as our $LCA$ operation.
...
\todo{Describe the rest of the chapter and report time complexity}
